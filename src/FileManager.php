...
use Elinkcms\ElinkLaravelFileManager\Events\Deleted;
use Elinkcms\ElinkLaravelFileManager\Traits\CheckTrait;
use Elinkcms\ElinkLaravelFileManager\Traits\ContentTrait;
use Elinkcms\ElinkLaravelFileManager\Traits\PathTrait;
use Elinkcms\ElinkLaravelFileManager\Services\TransferService\TransferFactory;
...
class FileManager
{
    ...
    public function upload($disk, $path, $files, $overwrite)
    {
        ...
        foreach ($files as $file) {
            ...
            event(new AfterGetFileName($fileName));
    ...
    {
        $newFileUrl = Storage::disk($disk)->url($newName);
        
        $fileinfo = pathinfo($newFileUrl);        
        $newExtension = $fileinfo['extension']?? null;

        $allowTypes = $this->configRepository->getAllowFileTypes();
        
        // folder
        if(!$newExtension){
            event(new AfterGetDirectoryName($newName));
        }
        
        if ($newExtension and !in_array($newExtension, $allowTypes)) {
            return [
                'result' => [
                    'status'  => 'warning',
                    'message' => $newExtension.' extension prohibited',
                ],
            ];
        }
    ...
    {
        event(new AfterGetDirectoryName($name));
